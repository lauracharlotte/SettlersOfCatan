...
public class DiscardCardsCommandTest
{
    ...
    public DiscardCardsCommandTest()
    {
    }
    ...
    public void testExecute() throws Exception
    {
        ...
        currentModel.getTurnTracker().setStatus(TurnStatusEnumeration.discarding);
        ...
        IJSONSerializable req = new DiscardCardsRequest(new PlayerIdx(0), new ResourceCards(3,0,3,0,3));
        String requestBody = req.serialize();
        ICommand command = new DiscardCardsCommand();
        command.execute(facade, requestBody, currentCookie);
        assert(manager.getGameWithNumber(0).getPlayers().iterator().next().getHand().getResourceCards().getTotal() == 6);
    }
